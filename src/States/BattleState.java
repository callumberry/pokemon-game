 ...
 *          you can attack, heal, or run away. Once you level up
 *          your pokemon enough you can take on the champion
 * Author:  Callum Berry
 * Version: 1.0
...
public class BattleState extends State{
    ...
    private int attackBtnX, attackBtnY, healBtnX, healBtnY, runBtnX, runBtnY, okButtonX, okButtonY;
    private int maxPlayerHp, maxFoeHp, foeHp, canRun, foeChoice, potion, numberOfTimesHealed, foeNumHealed;
    private int playerDamage, foeDamage, power, foeExperience;
    private double playerAttack, playerDefence, foeAttack, foeDefence, aOverD;
    private boolean playerTurn, over, firstTurn;
    private BufferedImage playerPokemonSprite, foePokemonSprite;
    private String eventString, eventString2, playerPokemonName, foePokemonName;
    ...
    public BattleState(Game game, Display display){
        ...
        foePokemonName = (String) game.getPokemon().getWildPokemon()[0];
        foePokemonSprite = (BufferedImage) game.getPokemon().getWildPokemon()[1];
        foeAttack = game.getPokemon().getFoeAttackStat();
        foeDefence = game.getPokemon().getFoeDefenceStat();
        maxFoeHp = game.getPokemon().getFoeHPStat();
        foeExperience = (int)game.getPokemon().getWildPokemon()[6];
        foeHp = maxFoeHp;       //Sets the foeHp to it's maxHP
        ...
        runBtnX = game.getWidth() - width - 50;
        runBtnY = game.getHeight() - height- 50;
    ...
    public void update() {
        ...
        if(game.getMouseInput().isLeftPressed()){
            ...
            if(playerTurn){
                ...
                //Checks to see if the area of the run button is clicked
                if (mouseX >= runBtnX && mouseX <= runBtnX + width) {
                    if (mouseY >= runBtnY && mouseY <= runBtnY + height) {
                        Assets.sfxClick();  //Plays the click sfx
                        runAway();  //Runs the runAway method
                        playerTurn = false; //Sets playerTurn to false;
                    }
                }
            }
    ...
    public void render(Graphics g){
        ...
        //Draws the player pokemon to the canvas
        ...
        //Checks to see if the wild pokemon is pikachu; if it is it renders the sprite slightly higher and smaller than the other sprites
        if(game.getPokemon().getWildPokemon() == game.getPokemon().getPikachu()){
            g.drawImage(foePokemonSprite, 3 * game.getWidth() / 4 - foePokemonSprite.getWidth() * 7, game.getHeight() / 2 - 3 * playerPokemonSprite.getHeight() * 8 / 4 - 25,
                    foePokemonSprite.getWidth() * 7, foePokemonSprite.getHeight() * 7, null);
        }
        //If the wild pokemon isn't pikachu it renders it normally
        else{
            g.drawImage(foePokemonSprite, 3 * game.getWidth() / 4 - foePokemonSprite.getWidth() * 8, game.getHeight() / 2 - 3 * playerPokemonSprite.getHeight() * 8 / 4,
                    foePokemonSprite.getWidth() * 8, foePokemonSprite.getHeight() * 8, null);
        }

        //Checks to see if it's the player's turn
        if(playerTurn) {
            //Draws the attack, heal, and run button at the same spot as where the corresponding area that checks if its been clicked
            ...
            g.drawImage(Assets.runBtn, runBtnX, runBtnY, width, height, null);
        }
        else{
        ...
        //Draws the string with the wild pokemon's name, level, and HP to the canvas
        g.drawString(foePokemonName + "   " + "Lv" + game.getPokemon().getFoeLevel(), 3 * game.getWidth() / 4 - foePokemonSprite.getWidth() * 8, 100);
        g.drawString("Foe Hp: " + foeHp + "/" + maxFoeHp, 3 * game.getWidth() / 4 - foePokemonSprite.getWidth() * 8, 200);
    ...
    public void playerAttack(){     //playerAttack method
        aOverD = playerAttack / foeDefence;     //Takes the player's attack divided by the foe's defence
        ...
        foeHp -= playerDamage;      //Subtracts the player's damage from the foe's hp
    ...
    public void foeFight(){     //foeFight method
        aOverD = foeAttack / playerDefence;  //Takes the foe's attack divided by the players's defence
        foeDamage = (int)(((2 * game.getPokemon().getPlayerLevel()) / 5 + 2) * power * aOverD) / 50 + 2;  //Formula for calculating the damage done by the foe's pokemon

        State.setCurrentPlayerHP(State.getCurrentPlayerHP() - foeDamage); //Subtracts the foe's damage from the player's hp
        Assets.sfxAttack(); //Plays the attack sfx
        winCheck();     //Rns the winCheck method
        eventString = "Wild " + foePokemonName + " Attacks";//Sets the eventString to say the foe attacks
    }
    public void foeHeal(){ //foeHeal method
        foeHp += potion;  //Adds the value of a potion to the foe's HP
        Assets.sfxHeal(); //Plays heal sfx

        //Check to see is the foe's HP has surpassed the max HP it can have
        if(maxFoeHp <= foeHp){
            foeHp = maxFoeHp;//Sets the foe's HP to the max HP
        }

        eventString = "Wild " + foePokemonName + " uses";   //Sets the eventString to say the foe heals
        eventString2 = "a Sitrus Berry";                    //Sets the eventString2 to say the foe heals
    }

    public void runAway(){      //runAway method
        //Creates a random number
        canRun = (int)(Math.random() * 2);

        //Checks to see if the random number created is equal to 1
        if (canRun == 1) {
            State.setRunAway(true);     //Sets boolean runAway to true
            game.getMouseInput().setLeftPressed(false);         //Sets boolean leftPressed to false

            //Changes the variable for experienceGained and totalExperience
            State.setExperienceString("Experience Gained: " + getExperienceGained());
            State.setExperienceString2("Total Experience: " + State.getTotalExperience());

            State.setState(game.postBattleState);       //Switches to the postBattleState
            over = true;        //Sets over to true
        }
        else{
            eventString = "You Couldn't Escape";        //Sets the event string to inform the player they couldn't run away
        }
    }

    public void experienceGained(){     //experienceGained method
        State.setExperienceGained((int)((double)foeExperience * (double)game.getPokemon().getFoeLevel()) / 7);      //Formula for the experience gained; take in the wild pokemon's level and their base experience points
    ...
    public void winCheck(){     //winCheck method
        ...
        if(foeHp <= 0){     //Win
            ...
            State.setState(game.postBattleState);       //Sets the game state to the postBattleState
    ...
    public void resetBattle(){ //resetBattle method
        ...
        foeHp =  maxFoeHp;      //Sets the foe's HP to the max
    ...
    public void changeWildPokemon(){//ChangeWildPokemon method
        game.getPokemon().wildPokemonChooser();     //Runs the wildPokemonChooser method
        game.getPokemon().wildPokemonStatCalc();        //Runs the wildPokemonStatCalc method
        ...
        foeHp = maxFoeHp;       //Sets the value of foeHp to maxFoeHp
    }
}
